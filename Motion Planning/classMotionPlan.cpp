...
class commonVars{
protected:
	static const int IDIM = 8; // horizontal size of the squares
	static const int JDIM = 8; // vertical size size of the squares
	static const int NDIR = 4; // number of possible directions to go at any position
	...
	commonVars()
	{
		iDir[0] = 1; iDir[1] = 0; iDir[2] = -1; iDir[3] = 0;

		jDir[0] = 0; jDir[1] = 1; jDir[2] = 0; jDir[3] = -1;
		// if NDIR = 8
...
class motionPlan : public commonVars
{
private:
	int iStart, jStart;
	int iEnd, jEnd;
...
public:
	...
	motionPlan(int xStart, int yStart, int xEnd, int yEnd);
	string FindPath();
};

motionPlan::motionPlan(int xStart, int yStart, int xEnd, int yEnd)
{
...
string motionPlan::FindPath()
{
	return pathFind(Location(iStart, jStart), Location(iEnd, jEnd));
}
